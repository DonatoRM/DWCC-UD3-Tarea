...
function firstFunction() {
  ...
  if (newWindow == null) {
    ...
    let javaAvailability = newWindow.navigator.javaEnabled() ? "Sí" : "No"; // Usamos el operador ternario para comprobar si está habilitada JVM
...
do {
  ...
  let nameSurname = prompt("introduzca su nombre y apellidos").trim();
...
do {
  ...
  if (!isNaN(Date.parse(dateOfBirthUSA))) {
...
result += `<li>La primera letra A de tu nombre está en la posición: ${fullName.toLowerCase().indexOf("a")}</li>`;
result += `<li>La última letra A de tu nombre está en la posición: ${fullName.toLowerCase().lastIndexOf("a")}</li>`;
...
result += `<li>Naciste un feliz ${
  daysOfTheWeek[objectDateOfBirth.getDay()]
